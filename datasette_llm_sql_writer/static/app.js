...
function getStorageKey(db, table) {
  return `llm_sql_writer:${db || ''}/${table || ''}`;
}

function loadHistory(db, table) {
  try {
    const raw = localStorage.getItem(getStorageKey(db, table));
    if (!raw) return [];
...
function saveHistory(db, table, history) {
  try {
    localStorage.setItem(getStorageKey(db, table), JSON.stringify(history || []));
  } catch (_) { }
}
...
function renderPanel(node) {
  ...
  const history = loadHistory(db, table);
  ...
  insertBtn.textContent = 'Insert SQL only';
  insertBtn.style.marginLeft = '6px';
  const runBtn = document.createElement('button');
  runBtn.textContent = 'Insert and Run';
  ...
  async function generate() {
    ...
    if (status === 200 && json.sql) {
      ...
      saveHistory(db, table, newHistory);
      chatLog.value = newHistory.map(m => `${m.role.toUpperCase()}: ${m.content}`).join('\n');
  ...
  insertBtn.addEventListener('click', insertOnly);
  runBtn.addEventListener('click', insertAndRun);
}
...
document.addEventListener('datasette_init', function (ev) {
  ...
  // Only register if we appear to be on a table page
  const { db, table } = getDbAndTableFromPath();
  if (!db || !table) return;
